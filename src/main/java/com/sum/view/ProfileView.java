...
public class ProfileView {
    ...
    private User user;
    private Button save;
    private Button moodButton; // 我的心情按钮
    ...
    public ProfileView() {
        ...
        // 创建我的心情按钮
        this.moodButton = new Button("我的心情");
        this.moodButton.setCursor(Cursor.HAND);
        FontIcon moodIcon = new FontIcon("codicon-heart");
        moodIcon.setIconSize(16);
        moodIcon.setIconColor(Paint.valueOf("#FFF"));
        this.moodButton.setGraphic(moodIcon);
        this.moodButton.setStyle("-fx-background-color: #FF6B6B; -fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 16px; -fx-padding: 10 20; -fx-background-radius: 5;");
        
        // 创建心情按钮的容器
        HBox moodBox = new HBox();
        moodBox.setPadding(new Insets(10, 0, 30, 0));
        moodBox.setAlignment(Pos.CENTER);
        moodBox.getChildren().add(this.moodButton);
        
        // 添加按钮点击事件
        setupMoodButtonAction();
        
        // 将所有元素添加到布局中
        card.getChildren().addAll(avatar.getMainLayout(), gridPane, saveBox, moodBox);
     ...
     * 设置心情按钮的点击事件
     */
    private void setupMoodButtonAction() {
        this.moodButton.setOnAction(event -> {
            try {
                // 获取MainApp实例
                ApplicationContext context = JavaFxSMApplication.getApplicationContext();
                MainApp mainApp = context.getBean(MainApp.class);
                
                // 设置当前用户ID（确保使用正确的用户ID）
                mainApp.setCurrentUserId(this.user.getId());
                
                // 获取当前窗口，关闭它
                Stage currentStage = (Stage) this.moodButton.getScene().getWindow();
                currentStage.close();
                
                // 创建新的Stage实例并显示心情选择页面
                Stage newStage = new Stage();
                mainApp.setPrimaryStage(newStage);
                newStage.setTitle("选择心情");
                
                // 初始化并显示心情选择页面
                MoodSelectView moodSelectView = context.getBean(MoodSelectView.class);
                moodSelectView.initialize(mainApp);
                newStage.setScene(moodSelectView.getScene());
                newStage.centerOnScreen();
                newStage.show();
            } catch (Exception e) {
                e.printStackTrace();
                System.err.println("无法打开心情功能: " + e.getMessage());
            }
        });
    }
